...
// display company name-start

const companyName = products.map(function (value, index) {

  return `<button class="company-btn" onclick="filter_1('${value.company}')">${value.company}</button>`;

});

const newCompanyName = [...new Set(companyName)];

newCompanyName.unshift(`<button class="company-btn" onclick="filter_1('All')">All</button>`);

document.getElementById("companies").innerHTML = newCompanyName.join("");

// display company name-end
